...
def main():
	...
	server_nic_list = [ s for s in sorted(servers.values(), cmp=compare_server_id) if s['serverNIC'] == '0' ]
	...
	for server in servers:
		...
		for subnet in subnets:
			if ip_address not in subnet:
				print >>sys.stderr, "The number of servers found do not fit in the IP range provided. Either change the first-ip option, or use a larger subnet mask."
				sys.exit(1)
